...
const PriceCard: React.FC<PriceCardProps> = ({ item }) => {
  ...
  // Badge styling & Gradient styling (using darker text color for gradient base)
  let badgeColor = 'bg-slate-100 text-slate-700';
  let gradientClass = 'from-slate-700/10';
  ...
  switch (item.category) {
    ...
    case 'lubricant': 
      ...
      gradientClass = 'from-blue-700/10';
      break;
    case 'energy': 
      badgeColor = 'bg-red-100 text-red-700'; 
      gradientClass = 'from-red-700/10'; 
      break;
    case 'agriculture': 
      badgeColor = 'bg-green-100 text-green-700'; 
      gradientClass = 'from-green-700/10'; 
      break;
    case 'commodity': 
      badgeColor = 'bg-yellow-100 text-yellow-700'; 
      gradientClass = 'from-yellow-700/10'; 
      break;
  }

  return (
    <div className="bg-white rounded-xl shadow-sm border border-slate-100 hover:shadow-md transition-shadow duration-200 flex flex-col h-full animate-fade-in relative overflow-hidden">
      {/* Subtle Top Gradient using darker shade */}
      <div className={`absolute top-0 left-0 right-0 h-20 bg-gradient-to-b ${gradientClass} to-transparent pointer-events-none`}></div>
